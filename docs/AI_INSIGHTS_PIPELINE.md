...
6. **Android-only by design.** `iosMain/.../PdfParser.kt` never constructs a `GemmaFallbackExtractor`, so `GrammarAwareParser` always runs Tiers 1–5 and 7 only on iOS — no Kotlin-Native inference runtime is wired up, and a real port is a separate multi-week initiative tracked in `docs/ai_insights_adoptgemma.md`, not undertaken here. (Earlier text here claimed MediaPipe "has no iOS artifact" — a scoped research spike found this was wrong: MediaPipe's `LlmInference` does ship an iOS/Swift CocoaPod, but the whole MediaPipe LLM Inference API is now maintenance-only/deprecated on *both* platforms; Google's supported successor, LiteRT-LM, has a genuine open-source Swift API and is the current best candidate — see `docs/ai_insights_adoptgemma.md` §15 for the three unresolved interop questions before any build-out.) The iOS `actual GemmaEngine` (`GemmaEngine.ios.kt`) exists only to satisfy the `expect`/`actual` contract and always returns `Result.failure` from `generateResponse` — it does **not** run real inference, so it must never be wired into `PlatformPdfParser` without first implementing a real runtime.
