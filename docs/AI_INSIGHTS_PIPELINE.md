...
```
LedgerRowItem         → shows AppColors.Warning Info icon when isFieldLowConfidence(fieldKey) || isFieldDiagnosed(fieldKey)
LedgerCorrectionDialog → inline edit dialog (per-field); renders diagnosticSuggestionFor(fieldKey) hint when non-null
