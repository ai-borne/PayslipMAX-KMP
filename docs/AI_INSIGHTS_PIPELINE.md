...
**One-stop reference (SSOT) for how the PCDA(O) payslip parser works end-to-end, how AI Insights are generated, and how all the pieces connect. Updated to reflect the full Token-IR re-architecture (Phases 0–6), the iOS/Android token-parity fixes, the date-primary grammar detection redesign, the raw/structured display-merge fix, the CI-enforced iOS/Android token- and structured-field-parity gate, the LiteRT-LM unification that brought real on-device Tier 6 Gemma inference to *both* platforms (replacing the deprecated MediaPipe runtime and the former iOS fail-stub), and the subsequent GCS-pipe → app-store-native download re-architecture that replaced that self-hosted Firebase-Hosting/GCS model cache with each store's own CDN — Play Asset Delivery on Android (live) and Apple On-Demand Resources on iOS (live, verified end-to-end on a physical device; replaced an earlier, unfinished Background Assets approach that needed a new Xcode extension target and App Group entitlement) — to eliminate a GCS-egress bill that scaled with install count (see [§10 SWOT](#10-swot-analysis) and [§11 Changelog](#11-changelog) for the story behind each).**
...
6. [Legacy String Path (Removed)](#6-legacy-string-path-removed)
...
6. **Now on both platforms (was Android-only).** Both `androidMain` and `iosMain` `PdfParser.kt` construct a `GemmaFallbackExtractor` when `resolveInstalledGemmaModelPath()` resolves a real path (Play Asset Delivery pack location on Android, main-bundle ODR resource path on iOS — `NSBundle.pathForResource` once `NSBundleResourceRequest` has fetched the "GemmaModel" tag); when no model is present, both return `null` and `GrammarAwareParser` runs Tiers 1–5 and 7 only (Tier 6 standby). iOS Tier 6 is live on real devices (verified end-to-end on physical hardware — see [§11 Changelog](#11-changelog)); the gate itself is additionally proven correct by `iosSimulatorArm64Test`. This closes the historical iOS gap: `GemmaEngine.ios.kt` used to always return `Result.failure` as a deliberate fail-stub, and is now a real bridge to LiteRT-LM Swift inference (it still fails loudly if the `inferenceDelegate` was never registered — a wiring bug, not a silent fake). **One honest caveat:** the Swift bridge compiling against the real LiteRT-LM API and the SPM package resolving are not gradle-verifiable (no gradle task builds the `.xcodeproj`); every Swift symbol was verified against Google's official docs, and end-to-end compile + real inference is the manual on-device smoke test. See `docs/ai_insights_adoptgemma.md` §15 for the full runtime architecture.
...
`fieldSource` is a provenance sidecar parallel to `fieldConfidence`: every field starts tagged `GEOMETRY` in `ReconciliationSolver.solve()`, and `applyGemmaFallback` (`GemmaFallbackMerger.kt`, called from `SharedParsingPipeline`) re-tags any key it merges in from Tier 6 to `GEMMA_FALLBACK`. This closes the SWOT weakness below — the UI/auditors can now tell "the geometry solver was certain" apart from "Gemma inferred this" instead of both collapsing into the same `fieldConfidence` number.
...
| Gemma Telemetry Opt-out | User control in Security & Privacy settings dynamically enables/disables model install tracking via `GemmaInstallTelemetry` (DB v10) |
...
| `telemetry/GemmaInstallTelemetry.kt` | Common interface and throttling/opt-out wrapper for Gemma model download and installation event analytics |
...
| `androidMain/.../telemetry/GemmaInstallTelemetry.android.kt` | Native Android implementation of Gemma install telemetry routing to Firebase Analytics and Crashlytics |
...
| `iosMain/.../telemetry/GemmaInstallTelemetry.ios.kt` | Native iOS hook delegating Gemma install telemetry calls to Swift-registered companion delegate |
...
| `androidUnitTest/.../GemmaInstallTelemetryIntegrationTest.kt` | Integration tests verifying database opt-out settings persistence, ViewModel integration, and UI telemetry switch behavior |
...
| `commonTest/.../telemetry/GemmaInstallTelemetryTest.kt` | Unit tests for Gemma install telemetry throttling, event formatting, and opt-out controls |
...
- ~~**Finish the legacy-path removal** — `PayslipTextParser`, `DynamicSpatialParser`, and dead `PayslipTokenParser` — once their remaining test dependents are migrated to the token/date-primary path.~~ **Closed** (July 2026): all three deleted, tests migrated to `GrammarAwareParser`. See [§6](#6-legacy-string-path-removed) and [§11 Changelog](#11-changelog).
...
- **iOS Tier 6 model delivery switched from Apple Background Assets to On-Demand Resources (ODR).** Background Assets required a new Xcode extension target, an App Group entitlement, and a self-hosted R2 manifest — all still-unbuilt work blocked on Apple Developer Program enrollment. ODR achieves the same "small initial download, fetch the ~557MB model later" goal using Apple's own CDN and asset-tag pipeline instead, removing that infrastructure entirely: `GemmaModelPaths.ios.kt` resolves the model via `NSBundle.pathForResource` once the "GemmaModel" ODR tag has been fetched, instead of an App Group container; `GemmaBaseModelInstaller.ios.kt`'s `install()` now actively triggers the fetch (ODR is app-initiated, unlike Background Assets' OS-autonomous scheduling) via a new `installTrigger` companion closure; and `GemmaOnDemandResourceBridge.swift` replaces the deleted `GemmaBackgroundAssetsBridge.swift`, wrapping `NSBundleResourceRequest` and reporting progress/completion into the existing Kotlin slots. **Verified end-to-end on a physical device**: the model loads from the ODR-fetched path and Tier 6 inference runs successfully against a real payslip parse. Known follow-up (tracked in [§12](#12-things-to-do)): progress reporting is currently a synthetic 0–1000 scale derived from `fractionCompleted` rather than genuine byte counts.
...
- ~~**iOS Background Assets Xcode-side work**~~ — **superseded.** iOS model delivery switched to Apple's On-Demand Resources (ODR) instead, which needed none of the Background Assets machinery (no extension target, no App Group entitlement) and is live, verified end-to-end on a physical device. See [§2 Stage 6](#stage-6--tier-6-offline-gemma-fallback-gemmafallbackextractor--gemmaengine) and [§11 Changelog](#11-changelog).
...
- **`GemmaOnDemandResourceBridge.swift`'s progress reporting is synthetic**, not genuine byte counts — it converts `NSBundleResourceRequest.progress.fractionCompleted` into a fixed 0–1000 scale rather than reporting real bytes-downloaded/total-bytes. Cosmetic only (the `Downloading(progress)` state's fraction is still accurate), but worth replacing with the real fraction/byte values `NSProgress` already exposes.
...
- ~~**Phase 5 cleanup** (delete the now-dead `gemmaModelStorageDir()` expect/actual...) waits on iOS Background Assets landing.~~ **Superseded.** `gemmaModelStorageDir()` is not dead: with the ODR switch it's now the permanent path for iOS's debug-build sideload fallback in `resolveInstalledGemmaModelPath()` (see [§2 Stage 6](#stage-6--tier-6-offline-gemma-fallback-gemmafallbackextractor--gemmaengine)), not a temporary Background-Assets-pending workaround — no longer a cleanup candidate.
...
- **Firebase anonymous auth removed (Sep 2026 tech-debt 3.1).** `AuthTokenProvider` (common/android/ios), the iOS launch-time anonymous sign-in, `AuthTokenFetcher`, `firebase-auth-ktx` and the `FirebaseAuth` Xcode product were built to send an ID token to the server-side Gemini Cloud Function. That path was retired when the cloud AI UI was removed (`3198c982`, Aug 2026), leaving the token provider with zero production callers. Firebase Core, Crashlytics and Analytics are unchanged. Any reference above to `AuthTokenProvider` as a Swift-bridge pattern is historical.
- **Room destructive-migration fallback removed (Sep 2026 tech-debt 2.2).** Both platform `getDatabaseBuilder()` actuals no longer call `fallbackToDestructiveMigration`, so an unmigratable or unknown DB version now fails loudly instead of silently wiping payslips (which exist only on-device). Every `@Database.version` bump must ship an `AutoMigration`/`Migration`; `PayslipDatabaseUpgradeTest` upgrades every exported schema version to head, checks a payslip row survives 5→11, and asserts an unrecognised version fails and keeps its data. iOS has no native migration test and relies on the shared migrations.
- **Gemma installer is now a single shared instance; overlapping ODR triggers collapsed (Sep 2026 tech-debt 1.2).** `GemmaBaseModelInstaller` was never Koin-injected: `PayslipViewModel` built one from its default argument, so each ViewModel got its own, and each iOS installer overwrote the companion-object `progressReporter`/`completionReporter` slots and orphaned the previous one's subscriber. `appModule` now registers it as a `single` and passes it in. `IosGemmaBaseModelInstaller.install()` also holds a `fetchInFlight` flag (cleared on completion, only set once `installTrigger` exists) so `PayslipViewModel.init` and `resumeModelDownload()` cannot make `GemmaOnDemandResourceBridge` replace its `NSBundleResourceRequest` mid-download. The companion slots and the Swift bridge are unchanged. `GemmaInstallerDiWiringTest` and two `IosGemmaBaseModelInstallerTest` cases cover it; the real-device ODR download check is pending the next TestFlight build.
