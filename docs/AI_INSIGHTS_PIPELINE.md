...
**One-stop reference (SSOT) for how the PCDA(O) payslip parser works end-to-end, how AI Insights are generated, and how all the pieces connect. Updated to reflect the full Token-IR re-architecture (Phases 0–6), the iOS/Android token-parity fixes, the date-primary grammar detection redesign, the raw/structured display-merge fix, the CI-enforced iOS/Android token- and structured-field-parity gate, and the LiteRT-LM unification that brings real on-device Tier 6 Gemma inference to *both* platforms (replacing the deprecated MediaPipe runtime and the former iOS fail-stub) via a version-aware dual-slot download backed by a Firebase-Hosting/GCS model cache (see [§10 SWOT](#10-swot-analysis) and [§11 Changelog](#11-changelog) for the story behind each).**
...
1. **Version-Aware Dual-Slot Storage & Download Lifecycle**: The quantized Gemma model binary (`gemma3-1b-it-int4.litertlm`, ~529MB — the generic CPU/GPU int4 LiteRT-LM build, the **identical file on both platforms**) is managed locally via `GemmaModelStorageManager`, which resolves an **active slot** (`gemma-active.litertlm`) and a **staging slot** (`gemma-staging.litertlm`) plus a small `gemma-active.version` metadata record (SSOT for "what version is on disk"). `GemmaModelVersionManager` fetches the version manifest (`{version,url,sha256,noticeText,noticeUrl}`) from the Firebase-Hosting-fronted GCS cache, attaching the interim shared-key header; `PayslipViewModelGemmaExtensions.setLocalAiEnabled` compares the manifest version to the recorded active version and, if different, streams the download via `GemmaModelDownloadManager` into the **staging** slot, verifies its SHA-256, and only then atomically promotes staging→active (rename overwrite) — a failed checksum leaves the old active slot untouched (the "keep Slot A active until Slot B is proven" rollback, now implemented; see `docs/ai_insights_adoptgemma.md` §10). The Gemma Terms-of-Use notice from the manifest is surfaced in the local-AI settings copy before download to satisfy the license's redistribution-notice requirement.
...
3. **Platform Binding & Runtime**: Both platforms verify active-slot readiness (`.endsWith(".litertlm")` + file exists) and instantiate `GemmaEngine` over the supported **LiteRT-LM** runtime. On Android, `GemmaEngine.android.kt` uses the `com.google.ai.edge.litertlm:litertlm-android` Kotlin `Engine`/`Conversation` API directly (Engine loads the model once; a fresh stateless `Conversation` runs per prompt). On iOS, `GemmaEngine.ios.kt` bridges through a registrable `inferenceDelegate` closure to a native Swift wrapper (`GemmaInferenceBridge.swift`) that calls LiteRT-LM's Swift Package `Engine`/`Conversation` API (Kotlin/Native cannot cinterop a pure-Swift package with no Objective-C headers, so the Swift-side bridge registered at app startup is mandatory — the same pattern as `AuthTokenProvider.ios.kt`). MediaPipe LLM Inference — the previous Android runtime — is maintenance-only/deprecated on both platforms and has been removed.
...
### Before production (currently in dev mode)

- **Provision `GEMMA_CACHE_KEY`** (the interim shared-key header the manifest endpoint validates — see [§10 Threats](#10-swot-analysis)) so the in-app dual-slot download pipeline actually works end-to-end instead of requiring a manual device sideload (how the real-hardware smoke test in [§11 Changelog](#11-changelog) obtained the model).
- **Upgrade the interim shared-key manifest auth to real Firebase App Check** (Play Integrity / App Attest) once both apps are properly registered/signed in Play Console and App Store Connect — blocked on store enrollment, not code.

### Future sprint (after the corpus backfill below reaches gold-standard parsing coverage)

- **Give Gemma's Tier 6 fallback a diagnostic-suggestion mode.** Today's prompt (`GemmaPromptBuilder`) only maps raw leftover line items to standardized keys — it never sees gross/net totals and is never asked to reconcile, so it structurally cannot catch or explain a numeric-extraction error elsewhere in the table (the exact situation in the parse-accuracy item below). A future iteration could hand Gemma the full classified item list plus target totals and ask it to *suggest* which field looks mis-extracted and why — surfaced to the user as a hint in the correction UI, never auto-applied. Auto-correcting financial figures via LLM is explicitly out of scope; this is diagnosis, not correction.
