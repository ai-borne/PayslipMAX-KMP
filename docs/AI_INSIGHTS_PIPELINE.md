...
10. **Redundant Currency Symbol UI Cleanup**:
    - **Visual Noise Reduction**: Removed redundant Rupee (`₹`) symbols from individual table row items inside `LedgerRowItem` in [LedgerSection.kt](file:///Users/test/Downloads/PDFParser/composeApp/src/commonMain/kotlin/com/ssbmax/pdfparser/ui/screens/LedgerSection.kt) to improve legibility and clean up the replica table.
    - **Header Integration**: Updated [AppStrings.kt](file:///Users/test/Downloads/PDFParser/composeApp/src/commonMain/kotlin/com/ssbmax/pdfparser/ui/theme/AppStrings.kt) to append `(₹)` to the Earnings and Deductions table column headers.
