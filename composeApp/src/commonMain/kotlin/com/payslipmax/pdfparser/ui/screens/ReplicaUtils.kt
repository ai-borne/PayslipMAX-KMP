...
private val ledgerBalanceKeys = setOf("openingCreditBalance", "closingDebitBalance", "openingDebitBalance", "closingCreditBalance")

/**
 * Standardized earnings fields and raw (unmatched) fields are populated disjointly by the parser
 * (see [com.payslipmax.pdfparser.parser.ReconciliationSolver.route]) — an item is never in both. So the
 * displayed ledger is always their union, never an either/or choice: a single stray unmatched item
 * (e.g. footer noise) must not hide an otherwise fully itemized structured breakdown.
 */
private fun structuredCreditLines(earnings: com.payslipmax.pdfparser.domain.Earnings): List<LedgerLine> =
    listOf(
...
private fun rawCreditLines(rawEarnings: Map<String, Double>): List<LedgerLine> =
    rawEarnings
        .filter { (key, value) ->
            value != 0.0 &&
                (PayslipPatternConfig.creditKeysMapping[key] ?: key) !in ledgerBalanceKeys &&
                isValidRawKey(key)
        }
...
private fun structuredDebitLines(deductions: com.payslipmax.pdfparser.domain.Deductions): List<LedgerLine> =
    listOf(
...
private fun rawDebitLines(rawDeductions: Map<String, Double>): List<LedgerLine> =
    rawDeductions
        .filter { (key, value) ->
            value != 0.0 &&
                (PayslipPatternConfig.debitKeysMapping[key] ?: key) !in ledgerBalanceKeys &&
                isValidRawKey(key)
        }
        .map { (key, value) -> LedgerLine(key, value, getDebitDesc(key), key) }

internal fun getDebitsList(payslip: ParsedPayslip): List<LedgerLine> {
    val items = structuredDebitLines(payslip.deductions) + rawDebitLines(payslip.rawDeductions)
