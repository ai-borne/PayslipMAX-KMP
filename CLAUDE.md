...
- The legacy `PayslipTokenParser`/`PayslipTextParser`/`DynamicSpatialParser` string path has been **deleted** (removed July 2026); `GrammarAwareParser` is the only production entry point on both platforms, and there is no secondary/legacy path left to avoid extending.
...
- **The JVM corpus suite proves correctness, not iOS performance.** `commonMain` code that's fast and correct under `java.util.regex` (Android/JVM) can still be catastrophically slow on Kotlin/Native's regex engine — a lookaround-regex construct in `ParserUtils.kt` was quadratic-or-worse on Native, causing a 4–11 minute iOS-only parse stall invisible to 52/52 green JVM tests (root cause and fix in the doc's changelog). Any new perf-sensitive `commonMain` string/regex hot path needs an `iosTest` timing assertion (see `ParserUtilsIosPerfTest`), not just a JVM correctness test — `iosSimulatorArm64Test` runs unconditionally in CI (`kmp-ios-ci`, macOS runner) so this is an automated gate, not a manual step.
